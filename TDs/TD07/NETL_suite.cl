...
(defun wave (mark type-arc direction) ;; avec direction :diret ou :inverse
  ...
  ;; algorithme
  ;; file = []
  ;; pour chaque node de *nodes*    ;; AJOUT DES NOEUDS INITIALEMENT MARQUES DANS PILE
  ;;    si is-marked (node mark)
  ;;          file.enfiler(node)
  ;;
  ;; tant que pile non vide         ;; PROPAGATION EN PROFONDEUR DU MARQUAGE
  ;;      actu = defiler(file)
  ;;      si direction == :direct
  ;;          pour chaque arc de actu.out-arcs
  ;;              si arc.type == type-arc ET (NON is-marked(arc.to mark))
  ;;                  mark(arc.to mark) 
  ;;                  file.enfiler(arc.to)
  ;;
  ;;      si direction == :inverse
  ;;          pour chaque arc de actu.in-arcs
  ;;              si arc.type == type-arc ET (NON is-marked(arc.from mark))
  ;;                  mark(arc.from mark) 
  ;;                  file.enfiler(arc.from)
